...
class DistributionTest extends Test {
    ...
    DistributionTest() {
        optionalDependsOn({ requiresDists }, ['all', 'bin', 'src'].collect { ":distributions:${it}Zip" })
        optionalDependsOn({ requiresBinZip }, ':distributions:binZip')
        optionalDependsOn({ requiresLibsRepo} ,':toolingApi:publishLocalArchives')
    }
    ...
    protected optionalDependsOn(Closure depend, Object... dependencies) {
        dependsOn { depend() ? dependencies : null }
    }
}
